...
feature
    : function_name=ID  LPAREN ( args+=formal ( COMMA args+=formal)* )? 
                        RPAREN COLON return_type=(TYPE | SELF_TYPE) LBRACE body=expr RBRACE   # function_definition
    | attribute_name=ID COLON attribute_type=(TYPE | SELF_TYPE) ( ASSIGN default=expr)?       # attribute_definition
    ;

formal
    : var_name=ID COLON type_name=(TYPE | SELF_TYPE)
    ;

let_entry
    : var_name=ID COLON var_type=(TYPE | SELF_TYPE) (ASSIGN default_value=expr)?
    ;
...
expr
    ...
    | NEW class_name=(TYPE | SELF_TYPE)                                                                     # new
    | ID                                                                                                    # id
    | INT                                                                                                   # int
    | STRING                                                                                                # string
    | BOOL                                                                                                  # bool
