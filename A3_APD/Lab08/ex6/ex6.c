...
int main (int argc, char *argv[])
{
    ...
    MPI_Comm_split(MPI_COMM_WORLD, old_rank/4, new_rank, &newcomm),
    ...
    MPI_Send(&new_rank, 1, MPI_INT, (new_rank+1)%4, 0, newcomm);

MPI_Status status;
    // Receive the rank.
    MPI_Recv(&recv_rank, 1, MPI_INT, (new_rank-1)%4, 0, newcomm, &status);
