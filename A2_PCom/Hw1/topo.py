...
def should_skip(testname):
    if static_arp():
        if testname in {"router_arp_reply", "router_arp_request"}:
            return True

    # print(len(sys.argv))
    # print(len(sys.argv) > 2)
    if len(sys.argv) > 2:
        if(testname=='forward' and 0<sum(['forward' in e for e in sys.argv[2:]])):
            return False
        if testname not in sys.argv[2:]:
            return True
