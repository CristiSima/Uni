...
void Lab6::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix)
{
    ...
    // (student): Get shader location for uniform mat4 "Model"
    int shader_location = glGetUniformLocation(shader->program, "Model");

    // (student): Set shader uniform "Model" to modelMatrix
    glUniformMatrix4fv(shader_location, 1, GL_FALSE, glm::value_ptr(modelMatrix));

    // (student): Get shader location for uniform mat4 "View"
    int view_location = glGetUniformLocation(shader->program, "View");

    // (student): Set shader uniform "View" to viewMatrix
    glm::mat4 viewMatrix = GetSceneCamera()->GetViewMatrix();
    glUniformMatrix4fv(view_location, 1, GL_FALSE, glm::value_ptr(viewMatrix));

    // (student): Get shader location for uniform mat4 "Projection"
    int projection_location = glGetUniformLocation(shader->program, "Projection");

    // (student): Set shader uniform "Projection" to projectionMatrix
    glm::mat4 projectionMatrix = GetSceneCamera()->GetProjectionMatrix();
    glUniformMatrix4fv(projection_location, 1, GL_FALSE, glm::value_ptr(projectionMatrix));

    // (student): Get shader location for uniform mat4 "Projection"
    int time_location = glGetUniformLocation(shader->program, "TimePassed");

    // (student): Set shader uniform "Projection" to projectionMatrix
    float timePassed = Engine::GetElapsedTime();
    glUniform1f(time_location, timePassed);
