...
void Lab2::Init()
{
    ...
    // by definition!
    {
        ...
        // Actually create the mesh from the data
        CreateMesh("tetraEDU", vertices, indices);
    }
    ...
    // opposing vertex orientations.
    {
        ...
        // Actually create the mesh from the data
        CreateMesh("PAtratu", vertices, indices);
    }
    {
        vector<VertexFormat> vertices
        {
        ...
        for(int i=1;i<=50;i++)
            vertices.push_back( VertexFormat(glm::vec3(sin((float)6.4f*i/50), 0,  cos((float)6.4f*i/50)), glm::vec3(0, 1, 0), glm::vec3(0.2, i/50, 0.6)));
        vector<unsigned int> indices =
        {
        };
        ...
        for(int i=1;i<=50;i++)
        {
            indices.push_back(0);
            indices.push_back(i);
            indices.push_back((i+1)%51);
        }
        // Actually create the mesh from the data
        CreateMesh("cerc", vertices, indices);
    }
...
void Lab2::Update(float deltaTimeSeconds)
{
    ...
    RenderMesh(meshes["tetraEDU"], shaders["VertexColor"], glm::vec3(1.5f, 1.5f, 0), glm::vec3(0.25f));

    // TODO(student): Draw the square

    RenderMesh(meshes["PAtratu"], shaders["VertexColor"], glm::vec3(1.5f, 1.5f, 2), glm::vec3(0.25f));
    // TODO(student): Disable face culling
    glDisable(GL_CULL_FACE);
    RenderMesh(meshes["cerc"], shaders["VertexColor"], glm::vec3(2, 0, 2), glm::vec3(0.25f));
}
