...
namespace implemented
{
    class Camera_T2
    {
     public:
        ...
        Camera_T2(const glm::vec3 &position, const glm::vec3 &center, const glm::vec3 &up)
        {
        ...
        // Update camera
		void Set(const glm::vec4 &position, const glm::vec4 &direction)
		{
			Set(
				glm::vec3(position-direction*distanceToTarget)+glm::vec3(0, distanceToTarget*sin(RADIANS(30)), 0),
				glm::vec3(position),
				glm::vec3(0, 1, 0)
			);

			// std::cout<<this->position<<"\t"<<this->forward<<"\n";
            // this->position = glm::vec3(position-direction);
        ...
        {
            // (student): Compute the new `forward` and `up` vectors.
            // Don't forget to normalize the vectors! Use `glm::rotate()`.
            glm::mat4 rotationMatrix=glm::rotate(glm::mat4(1.0), angle, glm::vec3(1, 0, 0));
            forward=glm::vec3(glm::normalize(rotationMatrix*glm::vec4(forward, 0.0)));
        ...
        void RotateFirstPerson_OY(float angle)
        {
            ...
            // vectors!
            glm::mat4 rotationMatrix=glm::rotate(glm::mat4(1.0), angle, glm::vec3(0, 1, 0));
            forward=glm::vec3(glm::normalize(rotationMatrix*glm::vec4(forward, 0.0)));
            right=glm::vec3(glm::normalize(rotationMatrix*glm::vec4(right, 0.0)));
        ...
        void RotateFirstPerson_OZ(float angle)
        {
            ...
            glm::mat4 rotationMatrix=glm::rotate(glm::mat4(1.0), angle, glm::vec3(0, 0, 1));
            right=glm::vec3(glm::normalize(rotationMatrix*glm::vec4(right, 0.0)));
        ...
        glm::mat4 GetMinimapViewMatrix()
        {
            // Returns the view matrix
            return glm::lookAt(position + forward+glm::vec3(0,10,0), position + forward, forward);
        }
