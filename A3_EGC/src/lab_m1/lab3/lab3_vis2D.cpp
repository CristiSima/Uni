...
void Lab3_Vis2D::Init()
{
    ...
    pressedA=pressedS=pressedD=pressedW=0;
    pressedZ=pressedX=0;
}
...
void Lab3_Vis2D::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (pressedA) logicSpace.x-=MOVE_DIRECTION*MOVE_DISTANCE*deltaTime;
    if (pressedD) logicSpace.x+=MOVE_DIRECTION*MOVE_DISTANCE*deltaTime;
    if (pressedW) logicSpace.y+=MOVE_DIRECTION*MOVE_DISTANCE*deltaTime;
    if (pressedS) logicSpace.y-=MOVE_DIRECTION*MOVE_DISTANCE*deltaTime;
...
void Lab3_Vis2D::OnKeyPress(int key, int mods)
{
    ...
    if(key=='Z')
    {
        logicSpace.x+=logicSpace.width/4;
        logicSpace.y+=logicSpace.height/4;
        logicSpace.width/=2;
        logicSpace.height/=2;
    }
