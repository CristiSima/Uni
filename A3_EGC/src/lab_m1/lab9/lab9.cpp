...
void Lab9::Update(float deltaTimeSeconds)
{
...

    {
        ...
        RenderSimpleMesh(meshes["box"], shaders["LabShader"], modelMatrix, mapTextures["crate"], mapTextures["grass"]);
    }
...
void Lab9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2)
{
    ...
    // (student): Set any other shader uniforms that you need
    float mix_factor= (texture1 && texture2)?0.5: 0;
    int loc_mix_factor = glGetUniformLocation(shader->program, "mix_factor");
    glUniform1f(loc_mix_factor, mix_factor);
