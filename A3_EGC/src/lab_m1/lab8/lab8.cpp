...
void Lab8::Init()
{
    ...
    // Light & material properties
    {
        ...
        lightDirections.push_back(glm::vec3(0, 0,-1));
        lightPositions.push_back(glm::vec3(0, 2, 0));
...
void Lab8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    int light_positions;
	light_positions = glGetUniformLocation(shader->program, "light_positions[0]");
	glUniform3f(light_positions, lightPositions[0].x, lightPositions[0].y, lightPositions[0].z);
	light_positions = glGetUniformLocation(shader->program, "light_positions[1]");
	glUniform3f(light_positions, lightPositions[1].x, lightPositions[1].y, lightPositions[1].z);

    int light_directions;
	light_directions = glGetUniformLocation(shader->program, "light_directions[0]");
	glUniform3f(light_directions, lightDirections[0].x, lightDirections[0].y, lightDirections[0].z);
	light_directions = glGetUniformLocation(shader->program, "light_directions[1]");
	glUniform3f(light_directions, lightDirections[1].x, lightDirections[1].y, lightDirections[1].z);
    ...
    int spotlight_enable_loc = glGetUniformLocation(shader->program, "spotlight_enable");
    glUniform1i(spotlight_enable_loc, (int)spotlight_enable);
...
void Lab8::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
		...
		if (window->KeyHold(GLFW_KEY_UP   )) lightDirections[0] = glm::vec3(rotate(glm::mat4(1), RADIANS(90)*deltaTime, glm::vec3(1, 0, 0))*glm::vec4(lightDirections[0], 1));
		if (window->KeyHold(GLFW_KEY_DOWN )) lightDirections[0] = glm::vec3(rotate(glm::mat4(1),-RADIANS(90)*deltaTime, glm::vec3(1, 0, 0))*glm::vec4(lightDirections[0], 1));
		if (window->KeyHold(GLFW_KEY_LEFT )) lightDirections[0] = glm::vec3(rotate(glm::mat4(1), RADIANS(90)*deltaTime, glm::vec3(0, 1, 0))*glm::vec4(lightDirections[0], 1));
		if (window->KeyHold(GLFW_KEY_RIGHT)) lightDirections[0] = glm::vec3(rotate(glm::mat4(1),-RADIANS(90)*deltaTime, glm::vec3(0, 1, 0))*glm::vec4(lightDirections[0], 1));

		#define SIMA_KEY_NUM_MINUS 333
		#define SIMA_KEY_NUM_PLUS  334
		if (((window->KeyHold(GLFW_KEY_LEFT_SHIFT) || window->KeyHold(GLFW_KEY_RIGHT_SHIFT)) && window->KeyHold(GLFW_KEY_EQUAL))
			|| window->KeyHold(SIMA_KEY_NUM_PLUS) ) cut_off_angle += 1;
		if (window->KeyHold(GLFW_KEY_MINUS)
			|| window->KeyHold(SIMA_KEY_NUM_MINUS)) cut_off_angle -= 1;
    }
}
