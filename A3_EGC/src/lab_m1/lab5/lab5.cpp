...
void Lab5::Update(float deltaTimeSeconds)
{
    ...
    // (student): Draw more objects with different model matrices.
    ...
    // implemented, and the local projection matrix.
    {
        ...
        modelMatrix=rotate(modelMatrix, RADIANS(45), glm::vec3(1, 0, 1));
        RenderMesh(meshes["box"], shaders["Simple"], modelMatrix);
    }{
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, glm::vec3(2, 0.5f, -2));
        modelMatrix = glm::scale(modelMatrix, glm::vec3(1, 20, 1));
        RenderMesh(meshes["box"], shaders["VertexNormal"], modelMatrix);
    }
