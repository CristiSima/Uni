...
(define (key n)
  (letrec ([cvartet (get-nth-quadruple n)]
            [e (second cvartet)] [f (third cvartet)]
           )
           (map (lambda (function) (modulo (function e f) 27))
...
(define (message->codes message)
  (map (lambda (char) (if (equal? char #\space)
                            0
...
(define (codes->message codes)
  (list->string (map (lambda (char-code)
                          (if (zero? char-code)
                              #\space
                              [integer->char (+ char-code 96)]
                          )
                     )
                     codes
                )
...
;; DAR CHIAR TREBUIE let?
(define (extend-key key size)
  (let ([key-len (length key)])
     (cond
        [(> key-len size) (take key size)]
        [(< key-len size) (extend-key (append key key) size)]
        [else key]
     )
  )
)
