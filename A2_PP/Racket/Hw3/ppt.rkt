...
(define (get-transformations n)
  (define (get-lowerst-3-mult n)
    (* 3 (quotient (- n 1) 3))
  )
  (define nr (- n 1))
  (if (zero? nr)
      null
      (append (get-transformations (+ (quotient (- nr 1) 3) 1))
              (cond
                 [(= (modulo nr 3) 1) '(1)]
                 [(= (modulo nr 3) 2) '(2)]
                 [else '(3)]
...
;  (if (null? Fs)
;      tuple
;      [apply-functional-transformations (cdr Fs) ((car Fs) tuple)]
;  )
  (foldl (lambda (function pair) (function pair))
         tuple
         Fs
  ) 
)
...
(define (get-nth-tuple n tuple Tr1 Tr2 Tr3)
...
(define (get-nth-ppt-from-matrix-transformations n)
  (define (multiply-with M)
       (lambda (V)
          (multiply M V)
       )
  )
  (get-nth-tuple n '(3 4 5) (multiply-with T1)
                            (multiply-with T2)
                            (multiply-with T3)
  )
)
;(get-nth-ppt-from-matrix-transformations 5)
...
(define (get-nth-quadruple n)
  (define (apply-wrapper function)
          (lambda (L) (apply function L))
  )
...
;(get-nth-ppt-from-GH-quadruples 5)