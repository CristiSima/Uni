...
(define ppt-stream-in-tree-order
  (let rec ([queue '((3 4 5))])
       (let* ( [tuple (car queue)] [queue (cdr queue)]   ;[tuple (force tuple)]
                [A (multiply T1 tuple)] [B (multiply T2 tuple)] [C (multiply T3 tuple)])
           (stream-cons tuple
                        (rec (append queue (list A B C)))
           )
       )
  )
)
...
(define (pairs G H)
  (let rec ([S H] [n 1])
      (let ([e (stream-first S)] [S (stream-rest S)])
           (stream-append (stream-map (lambda (x) (cons x e))
                                      (stream-take G n)
                          )
                          (stream-lazy (rec S (+ n 1)))
           )
      )
  )
)
...
(define gh-pairs-stream
  (stream-filter (lambda (args) (= (gcd (car args) (cdr args)) 1))
                 (pairs odds (stream-rest odds))
  )
)
...
;;    c = e^2 + f^2 = (h - g)^2 / 4 + (h + g)^2 / 4
;;      = (h^2 + g^2) / 2
  (stream-map (lambda (pair) (let ([g (car pair)] [h (cdr pair)])
                                         (list (* g h)
                                               (* (- h g) (+ h g) 1/2)
                                               (/ (+ (* h h) (* g g)) 2)
                                         )
