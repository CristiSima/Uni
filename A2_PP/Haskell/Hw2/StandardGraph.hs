...
outNeighbors node graph =  S.map (\edge -> snd edge)
                            $S.filter   (\edge -> node == fst edge)
                                        $edges graph
...
inNeighbors node graph = S.map (\edge -> fst edge)
                            $S.filter   (\edge -> node == snd edge)
                                        $edges graph
...
removeNode node graph = (S.delete node $nodes graph,
                         S.filter (\edge -> node /= snd edge && node /= fst edge) $edges graph)
...
splitNode old news graph =  (S.union (S.delete old (nodes graph)) (S.fromList news),
                             S.union (S.union
                                            (S.filter (\edge -> old /= snd edge && old /= fst edge) $edges graph)
                                            (S.cartesianProduct (inNeighbors old graph) (S.fromList news))
                                    )
                                    (S.cartesianProduct (S.fromList news) (outNeighbors old graph) )
                            )
...
mergeNodes :: Ord a
           ...
           -> StandardGraph a  -- graful existent
           -> StandardGraph a  -- graful obținut
mergeNodes prop node graph = if S.null (S.filter (\edge -> prop (snd edge) || prop (fst edge)) (edges graph))
                                then graph
                                else (  S.insert node (S.filter (not . prop) (nodes graph)),
                                        S.map   (\(src, dst) -> (if (prop src) then node else src,
                                                                 if (prop dst) then node else dst))
                                                $edges graph
                                     )
