...
-}
__search, __search_next :: Ord a
       => ([a] -> [a] -> [a])  -- funcția de îmbinare a listelor de noduri
       -> a                    -- nodul de pornire
       -> Graph a              -- graful
       -> [a]                  -- lista de noduri vizitate
       -> [a]                  -- ordinea vizitari urmatoarelor noduri
       -> [a]                  -- lista obținută în urma parcurgerii
__search f node graph visit_queue visited= node:__search_next f node graph (f   (tail visit_queue)
                                                                                [next | next <- (S.toList $outNeighbors node graph), not $elem next visited]) (node:visited)

__search_next f node graph visit_queue visited= if null visit_queue
                                            then []
                                            else __search f (head visit_queue) graph visit_queue visited
...
search :: Ord a
       ...
       -> Graph a              -- graful
       -> [a]                  -- lista obținută în urma parcurgerii
search f node graph = __search f node graph [node] []
...
countIntermediate from to graph = if (elem to $dfs from graph)
                                    then Just ( (M.fromJust $L.elemIndex  to (bfs from graph))-1,
                                                (M.fromJust $L.elemIndex  to (dfs from graph))-1)
                                    else Nothing
