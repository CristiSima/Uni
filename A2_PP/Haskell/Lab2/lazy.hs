...
findStringsLongerThanTenChars l = filter (\s -> length s > 10) l
...
setIntersection a b = [el| el<-a, elem el b]

setDiff :: Eq a => [a] -> [a] -> [a]
setDiff a b = [el| el<-a, not $ elem el b]

cartProduct :: [a] -> [b] -> [(a, b)]
cartProduct a b = [(ea,eb)| ea<-a, eb<-b]
...
newtonRaphson x g dg = infiniteApply (\s -> s -((g s)/(dg s))) x
