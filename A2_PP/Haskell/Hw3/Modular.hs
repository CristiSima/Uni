...
-}
mapSingle :: (a -> a) -> [a] -> [[a]]
mapSingle f xs = [  let (p1, _:p2)=splitAt x xs
                    in p1++[f $xs!!x]++p2           | x<-[0..((length xs)-1)]]
...
partitions [] = [[[]]]
partitions [x] = [[[x]]]
partitions (x:xs) =  [[x]:part|part<-partitions xs] ++ concat [mapSingle (x:) e| e<-(partitions xs)]
...
isModule :: Ord a
         ...
         -> Graph a
         -> Bool
isModule set graph =1 == (S.size $S.map (\node -> S.filter (\x -> not $elem x set) $outNeighbors node graph) set) &&
                    1 == (S.size $S.map (\node -> S.filter (\x -> not $elem x set) $ inNeighbors node graph) set)
...
isModularPartition :: Ord a
                   ...
                   -> Graph a
                   -> Bool
isModularPartition partition graph = S.fromList [True] == S.map (\set -> isModule set graph) partition
...
maximalModularPartition partitions graph = maximum $filter (\partition -> isModularPartition partition graph) $filter (\part -> S.size part /= 1) partitions
-- graph=diagram
-- graph=removeNode 5 diagram
-- parts = partitions $ S.toList $ nodes graph
--
-- partList = map (S.fromList . map S.fromList) parts
--
-- temp=filter (\partition -> isModularPartition partition graph) $filter (\part -> S.size part /= 1)partList
