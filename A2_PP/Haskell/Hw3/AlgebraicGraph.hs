...
instance Show a => Show (AlgebraicGraph a) where
    ...
    -- show (Connect a b) = S.union (S.cartesianProduct (nodes a) (nodes b)) $S.union (edges a) (edges b)
...
listToAlgebraicGraph :: [a] -> AlgebraicGraph a
listToAlgebraicGraph []=Empty
listToAlgebraicGraph [x]=Node x
listToAlgebraicGraph (x:xs)=Overlay (Node x) (listToAlgebraicGraph xs)
...
splitNode node targets = extend (\x ->  if x==node
                                        then listToAlgebraicGraph targets
                                        else Node x)
...
mergeNodes :: (a -> Bool)       -- proprietatea îndeplinită de nodurile îmbinate
           ...
           -> AlgebraicGraph a  -- graful existent
           -> AlgebraicGraph a  -- graful obținut
mergeNodes prop node = fmap (\x ->  if prop x
                                    then node
                                    else x)
...
filterGraph prop Empty = Empty
filterGraph prop (Node a) = if prop a then Node a else Empty
filterGraph prop (Connect a b) = Connect (filterGraph prop a) (filterGraph prop b)
filterGraph prop (Overlay a b) = Overlay (filterGraph prop a) (filterGraph prop b)
{-
    *** TODO ***
