...
instance Eq a => Eq (BST a) where
    (==) (BSTNil) (BSTNil) = True
    (==)    (BSTNod tree1_vl tree1_tl tree1_tr)
            (BSTNod tree2_vl tree2_tl tree2_tr) =   tree1_vl==tree2_vl &&
                                                    tree1_tl==tree2_tl &&
                                                    tree1_tr==tree2_tr
    (==) _ _ = False
...
instance Show a => Show (BST a) where
    -- show tree = let
    --                 _show dept BSTNil=""
    --                 _show dept (BSTNod vl tl tr) = let  __show 0=""
    --                                                     __show dept="\t" ++ (__show $dept-1)
    --                                                 in (__show dept)++(show vl)++"\n" ++
    --                                                     _show (dept+1) tl ++
    --                                                     _show (dept+1) tr
    --             in _show 0 tree
    show tree = let
                    _show prefix BSTNil=""
                    _show prefix (BSTNod vl tl tr) = prefix++(show vl)++"\n" ++
                                                    _show (prefix++"\t") tl ++
                                                    _show (prefix++"\t") tr
                in _show "" tree
...
instance Invertible (BST a) where
    ...
    invert (BSTNod vl tl tr)=(BSTNod vl (invert tr) (invert tl))
...
instance Functor BST where
    ...
    fmap f (BSTNod vl tl tr)=(BSTNod (f vl) (fmap f tl) (fmap f tr))
...
instance Foldable BST where
    ...
    foldr f acc (BSTNod vl tl tr)=foldr f (f vl (foldr f acc tr) ) tl
...
instance Container BST where
    contents tree = foldr (:) [] (invert tree)
